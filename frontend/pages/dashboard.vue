    ...
    <div>
        ...
        <div>
            ...
            <div v-if="g6loaded && categories.length">
                <div v-for="(category, i) in categories" :key="i" style="width:50%;display:inline-block">
                    <bar-chart
                        width="100%"
...
export default {
    ...
    data() {
        return {
            ...
            intl: new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }),
    ...
    methods: {
        async periodChanged() {
            this.periodNum = await this.$store.dispatch('graphs/periodReverse', this.month)
            this.period = await this.$store.dispatch('graphs/period', this.periodNum)
            await this.loadGraphs()
        },
        ...
        async loadGraph1() {
            ...
            this.revenueCY = []
            g1dataA.forEach(e => this.revenueCY.push({
                name: e.m,
                month: e.p,
                year: e.y,
                revenue: e.amount
            ...
            this.revenueLY = []
            g1dataB.forEach(e => this.revenueLY.push({
                name: e.m,
                month: e.p,
                year: e.y,
                revenue: e.amount
        ...
        async loadGraph1Totals() {
            ...
            if (periodLY >= 1) {
                this.totalRevenueLY = await this.$store.dispatch('graphs/graphOneTotals', {
                    location: this.location,
                    period: periodLY,
                    selector: "actuals"
                })
        ...
        async loadgraphSixIndividual() {
            ...
            this.categories.forEach(async(c) => {
                ...
                let g6data = await this.$store.dispatch(
                    'graphs/graphSixIndividual', {
                        location: this.location,
                        selector: this.targetSelector,
                        p: this.periodNum,
                        category: c['id']
        ...
        revCY(d) {
            ...
            f.forEach(e => {
                r.push({
                    name: e.m, month: e.mn, year: e.y, revenue: e.amount
                })
            })
            return r
        },
